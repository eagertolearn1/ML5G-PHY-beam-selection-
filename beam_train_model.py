...
opt = Adam()
bIsOldActivation =False
which_model='moe' # 'dense' or 'moe'
if multimodal == False:
    chain = 'complete'
else:
    chain = 'segment'
    if bIsOldActivation == True:
        Lastactivation = "relu"
    else:
        Lastactivation = "softmax"

if 'coord' in args.input:
    coord_model = modelHand.createArchitecture('coord_mlp',num_classes,[coord_train_input_shape[1]],chain)
if 'img' in args.input:
    if nCh==1:   
        img_model = modelHand.createArchitecture('light_image',num_classes,[img_train_input_shape[1],img_train_input_shape[2],1],chain)
    else:
        img_model = modelHand.createArchitecture('light_image',num_classes,[img_train_input_shape[1],img_train_input_shape[2],img_train_input_shape[3]],chain)
if 'lidar' in args.input:
    lidar_model = modelHand.createArchitecture('lidar_marcus',num_classes,[lidar_train_input_shape[1],lidar_train_input_shape[2],lidar_train_input_shape[3]],chain)

if multimodal == 2:
    if 'coord' in args.input and 'lidar' in args.input:
        combined_model = concatenate([coord_model.output,lidar_model.output])
        combined_model1 = Dense(num_classes,activation='relu')(combined_model)
        z = Dense(num_classes,activation=Lastactivation)(combined_model1)
        ...
        z = Dense(num_classes,activation=Lastactivation)(combined_model)
    ...
    #combined_model = tf.keras.layers.average()([lidar_model.output,img_model.output])

    Shapelidar= lidar_model.output.get_shape().as_list()
    Shapeimg  = img_model.output.get_shape().as_list()
    ShapeCoord= coord_model.output.get_shape().as_list()
    ShapeComb = combined_model.get_shape().as_list()


    shapelidartemp = lidar_model.output.get_shape()
    coord_weights = coord_model.get_weights()
    lidar_weights = lidar_model.get_weights()


    if which_model=='moe':
         n_experts = 3
         z = DenseMoE(num_classes,n_experts,expert_activation='relu', gating_activation='softmax')(combined_model)
